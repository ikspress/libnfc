...
int usbbus_prepare(void)
{
  ...
  if (!usb_initialized) {
    ...
    if (env_log_level && (((atoi(env_log_level) >> (NFC_LOG_GROUP_LIBUSB * 2)) &
    ...
    if (res != 0) {
      log_put(LOG_GROUP, LOG_CATEGORY, NFC_LOG_PRIORITY_ERROR,
              "Unable to init libusb (%s)", libusb_strerror(res));
      return res;
    }
      ...
      if (nfc_usb_devices[nfc_dev_idx].vendor_id == usbbus_get_vendor_id(dev) &&
        ...
        if (nfc_usb_devices[nfc_dev_idx].max_packet_size == 0) {
          ...
          for (size_t config_idx = 0; config_idx < get_usb_num_configs(dev); i++) {
            ...
            if (r != 0 || usb_config->interface == NULL ||
        ...
        printf("%s:%03d:%03d", "Test", dev_address, (int) valid_config_idx);
        size_t size_new_str =
            snprintf(connstrings[device_found],
                     sizeof(connstrings[device_found]), "%s:%03d:%03d",
                     usb_driver_name, dev_address, (int) valid_config_idx);
...
void usbbus_get_usb_endpoints(struct libusb_device *dev, uint8_t *endpoint_in,
...
void usbbus_get_usb_device_name(struct libusb_device *dev,
                                libusb_device_handle *udev, char *buffer,
                                size_t len)
{
  struct libusb_device_descriptor descriptor;
  libusb_get_device_descriptor(dev, &descriptor);
  if (descriptor.iManufacturer || descriptor.iProduct) {
    if (udev) {
      libusb_get_string_descriptor_ascii(udev, descriptor.iManufacturer & 0xff,
                                         (unsigned char *)buffer, len);
      if (strlen(buffer) > 0) {
        strncpy(buffer + strlen(buffer), " / ", 4);
      }
      libusb_get_string_descriptor_ascii(
          udev, descriptor.iProduct & 0xff,
          (unsigned char *)buffer + strlen(buffer), len - strlen(buffer));
    }
  }
}
